...
def __analyze(writer, args):
    ...
    if (".." in args.apk_file):
        raise ExpectedException("apk_file_name_slash_twodots_error",
                                u"APK 파일 이름에는 슬래시(/) 또는 두 개의 점(...)을 사용할 수 없습니다. (File: " + apk_Path + ").")
    ...
    if allurls_strip_non_duplicated_final_prerun_count != 0:
        writer.startWriter("SSL_URLS_NOT_IN_HTTPS", LEVEL_CRITICAL, u"SSL Connection 검사",
                           u"SSL을 사용하지 않는 URLs입니다. (Total:" + str(
        ...
        writer.startWriter("SSL_URLS_NOT_IN_HTTPS", LEVEL_INFO, u"SSL Connection 검사",
                           u"SSL을 사용하지 않는 URLs을 발견하지 못헀습니다. (알림: 만약 URL 문자열을 암호화했다면, 그것을 발견할 수 없습니다.).",
                           ["SSL_Security"])
    ...
    if args.extra == 2:  # The output may be too verbose, so make it an option
        ...
        if list_security_related_methods:
            ...
            writer.startWriter("Security_Methods", LEVEL_INFO, u"Security Methods 검사",
                               u"보안 관련 문자열을 포함하는 메서드 이름을 검색하지 못했습니다.")
    ...
    if args.extra == 2:  # The output may be too verbose, so make it an option
        ...
        if list_security_related_classes:
            ...
            writer.startWriter("Security_Classes", LEVEL_INFO, u"Security Classes 검사",
                               u"보안 관련 문자열을 포함하는 클래스 이름을 검색하지 못했습니다.")
    ...
    if isMasterKeyVulnerability:
        writer.startWriter("MASTER_KEY", LEVEL_CRITICAL, u"마스터 키 유형 I 취약성", u"이 APK에는 마스터 키 유형 I 취약성이 있습니다.", None,
                           "CVE-2013-4787")
    else:
        writer.startWriter("MASTER_KEY", LEVEL_INFO, u"마스터 키 유형 I 취약성", u"이 APK에는 마스터 키 유형 I 취약성이 없습니다.", None,
                           "CVE-2013-4787")
    ...
    if is_debug_open:
        writer.startWriter("DEBUGGABLE", LEVEL_CRITICAL, u"Android Debug Mode 검사",
                           u"AndroidManifest.xml에서 DEBUG 모드가 ON(android:debugable=\"true\")입니다. 이것은 매우 위험합니다. 공격자는 로그캣을 통해 디버그 메시지를 탐지할 수 있습니다. 릴리스된 응용 프로그램인 경우 DEBUG 모드를 비활성화하십시오.",
                           ["Debug"])

    else:
        writer.startWriter("DEBUGGABLE", LEVEL_INFO, u"Android Debug Mode 검사",
                           u"AndroidManifest.xml에서 DEBUG 모드가 OFF(android:debugable=\"false\")입니다.", ["Debug"])
    ...
    if list_detected_FLAG_DEBUGGABLE_path:
        writer.startWriter("HACKER_DEBUGGABLE_CHECK", LEVEL_NOTICE, u"Checking Android Debug Mode 검사를 위한 코드",
                           u"AndroidManifest.xml에서 \"ApplicationInfo.FLAG_DEBUGGABLE\" 검사하는 코드를 발견:",
                           ["Debug", "Hacker"])

        for path in list_detected_FLAG_DEBUGGABLE_path:
            writer.show_single_PathVariable(d, path)
    else:
        writer.startWriter("HACKER_DEBUGGABLE_CHECK", LEVEL_INFO, u"Checking Android Debug Mode 검사를 위한 코드",
                           u"AndroidManifest.xml에서 \"ApplicationInfo.FLAG_DEBUGGABLE\" 검사하는 코드를 발견하지 못했습니다.",
                           ["Debug", "Hacker"])
    ...
    if ACCESS_MOCK_LOCATION in all_permissions:
        writer.startWriter("USE_PERMISSION_ACCESS_MOCK_LOCATION", LEVEL_CRITICAL, u"불필요한 권한 검사",
                           u"권한 'android.permission.ACCESS_MOCK_LOCATION'은 에뮬레이터 환경에서만 작동합니다. 릴리스된 응용 프로그램인 경우 이 권한을 제거하십시오.")
    else:
        writer.startWriter("USE_PERMISSION_ACCESS_MOCK_LOCATION", LEVEL_INFO, u"불필요한 권한 검사",
                           u"권한 'android.permission.ACCESS_MOCK_LOCATION' 올바르게 설정되어 있습니다.")
    ...
    if permissionNameOfWrongPermissionGroup:  # If the list is not empty
        ...
        writer.startWriter("PERMISSION_GROUP_EMPTY_VALUE", LEVEL_INFO, u"AndroidManifest 권한그룹 검사",
                           u"PermissionGroup in permission tag of AndroidManifest sets correctly.")
    ...
    if list_user_permission_critical_manufacturer or list_user_permission_critical:
        if list_user_permission_critical_manufacturer:
            writer.startWriter("USE_PERMISSION_SYSTEM_APP", LEVEL_CRITICAL, u"AndroidManifest System 사용 권한 검사",
                               u"이 앱은 개발업체 또는 Google에서 릴리스하고 서명하며 '/system/app' 아래에 있어야 합니다. 그렇지 않으면 악성 앱일 수 있습니다.")
        ...
        if list_user_permission_critical:
            writer.startWriter("USE_PERMISSION_CRITICAL", LEVEL_CRITICAL, u"AndroidManifest System 사용 권한 검사",
                               u"이 앱은 매우 높은 권한을 가지고 있습니다.")

            for permission in list_user_permission_critical:
                writer.write("Critical use-permission found: \"" + permission + "\"")
    else:
        writer.startWriter("USE_PERMISSION_SYSTEM_APP", LEVEL_INFO, u"AndroidManifest System 사용 권한 검사",
                           u"시스템의 중요한 사용 권한을 찾을 수 없습니다.")
...
You are now allowing minSdk버전을 8보다 낮은 버전을 사용하고 있습니다. Google 클라우드 메시징(푸시 메시지) 서비스는 Android SDK > = 8(Android 2.2)만 허용합니다. 참조: http://developer.android.com/about/dashboards/index.html."""
    ...
    if organized_list_base64_success_decoded_string_to_original_mapping:  # The result is from the upper code section
        ...
        writer.startWriter("HACKER_BASE64_STRING_DECODE", LEVEL_CRITICAL, u"Base64 문자열 인코딩",
                           u"Found Base64 인코딩 발견 \"String(s)\" (총: " + str(
                               len(organized_list_base64_success_decoded_string_to_original_mapping)) + "). 모든 문자열이 Base64 인코딩이라고 보장할 수 없으며 디코딩된 바이너리 파일도 보여드리지 않습니다.:",
                           ["Hacker"])
        ...
        if list_base64_decoded_urls:
            ...
            writer.startWriter("HACKER_BASE64_URL_DECODE", LEVEL_CRITICAL, u"Base64 문자열 인코딩",
                               u"모든 문자열로 부터 Base64 인코딩 \"HTTP URLs without SSL\" (총: " + str(
        ...
        writer.startWriter("HACKER_BASE64_STRING_DECODE", LEVEL_INFO, u"Base64 문자열 인코딩",
                           u"인코딩된 Base64 문자열 또는 URL을 찾을 수 없습니다.", ["Hacker"])
...
아래 코드를 수정하세요.:"""
        ...
        writer.startWriter("WEBVIEW_RCE", LEVEL_INFO, u"WebView RCE 취약점 검사",
                           u"WebView addJavascriptInterface 취약점을 찾을 수 없습니다.", ["WebView", "Remote Code Execution"],
                           "CVE-2013-4710")
    ...
    if (not list_no_pwd_keystore) and (not list_protected_keystore) and (not list_no_pwd_probably_ssl_pinning_keystore):
        ...
        if list_no_pwd_probably_ssl_pinning_keystore:
            ...
            writer.startWriter("HACKER_KEYSTORE_SSL_PINNING", LEVEL_CRITICAL, u"KeyStore 보호 검사",
                               u"아래의 키 저장소는 SSL 고정 작업을 위해 \"byte array\" 또는 \"hard-coded cert info\"를 사용하는 것 같습니다. (총: " + str(
                                   len(list_no_pwd_probably_ssl_pinning_keystore)) + "). 수동으로 확인하십시오:",
                               ["KeyStore", "Hacker"])
        ...
        if list_no_pwd_keystore:
            ...
            writer.startWriter("HACKER_KEYSTORE_NO_PWD", LEVEL_CRITICAL, u"KeyStore 보호 검사",
                               u"아래 키 저장소는 암호로 보호되는 \"NOT\"인 것 같습니다. (총: " + str(
                                   len(list_no_pwd_keystore)) + "). 수동으로 확인하십시오:", ["KeyStore", "Hacker"])
        ...
        if list_protected_keystore:
            ...
            writer.startWriter("HACKER_KEYSTORE_SSL_PINNING2", LEVEL_NOTICE, u"KeyStore 보호 검사",
                               u"아래의 키 저장소는 암호로 \"protected\"되어 있으며 SSL-pinning을 사용하는 것 같습니다. (총: " + str(
                                   len(list_protected_keystore)) + "). \"Portecle\" 도구를 사용하여 KeyStore의 인증서를 관리할 수 있습니다:",
                               ["KeyStore", "Hacker"])
    ...
    if list_keystore_file_name or list_possible_keystore_file_name:
        if list_keystore_file_name:
            writer.startWriter("HACKER_KEYSTORE_LOCATION1", LEVEL_NOTICE, u"KeyStore 파일 위치", u"BKS Keystore 파일:",
                               ["KeyStore", "Hacker"])
            for i in list_keystore_file_name:
                writer.write(i)

        if list_possible_keystore_file_name:
            writer.startWriter("HACKER_KEYSTORE_LOCATION2", LEVEL_NOTICE, u"가능성있는 KeyStore 파일 위치",
                               u"가능성있는 BKS keystore 파일:", ["KeyStore", "Hacker"])
            for i in list_possible_keystore_file_name:
                writer.write(i)
    else:
        writer.startWriter("HACKER_KEYSTORE_LOCATION1", LEVEL_INFO, u"KeyStore 파일 위치",
                           u"가능성있는 BKS 키 저장소 또는 인증서 키 저장소 파일을 찾지 못했습니다(알림: 이 앱이 keystore를 사용하지 않는 것은 아닙니다.):",
                           ["KeyStore", "Hacker"])
    ...
    if list_Non_BKS_keystore:
        writer.startWriter("KEYSTORE_TYPE_CHECK", LEVEL_CRITICAL, u"KeyStore Type 검사",
                           u"Android는 'BKS' 유형 KeyStore만 허용합니다. 'BKS' 유형 키스토어를 사용하고 있는지 확인하십시오.:", ["KeyStore"])
    ...
    if list_PackageInfo_signatures:
        writer.startWriter("HACKER_SIGNATURE_CHECK", LEVEL_NOTICE, u"서명 코드 가져오기 검사",
                           u"이 앱은 코드에 패키지 서명을 확인하는 코드가 있습니다. 공격자들에 의해 앱이 해킹당했는지 확인하기 위해 사용될 수 있습니다.",
                           ["Signature", "Hacker"])
        for signature in list_PackageInfo_signatures:
            writer.show_Path(d, signature)
    else:
        writer.startWriter("HACKER_SIGNATURE_CHECK", LEVEL_INFO, u"서명 코드 가져오기 검사", u"이 앱이 코드에서 서명을 확인하는 것을 감지하지 못했습니다.",
                           ["Signature", "Hacker"])
    ...
    if list_code_for_preventing_screen_capture:
        ...
        writer.startWriter("HACKER_PREVENT_SCREENSHOT_CHECK", LEVEL_INFO, u"스크린샷 캡처 방지",
                           u"이 앱에 스크린샷 캡처를 차단하는 코드 설정이 있는지 탐지하지 못했습니다.", ["Hacker"])
    ...
    if path_Runtime_exec:
        writer.startWriter("COMMAND", LEVEL_CRITICAL, u"Runtime exec 검사",
                           u"이 앱은 critical한 함수 'Runtime.getRuntime().exec(\"...\")'를 사용하고있습니다.\n다음 코드 세션이 유해하지 않은지 확인하십시오:",
                           ["Command"])

        writer.show_Paths(d, path_Runtime_exec)

        if list_Runtime_exec:
            writer.startWriter("COMMAND_SU", LEVEL_CRITICAL, u"Runtime Critical Command 검사",
                               u"\"root\" 권한 코드 섹션 'Runtime.getRuntime(.exec(\"su\")'에 대한 요청 중입니다:", ["Command"])

            for path in list_Runtime_exec:
                writer.show_Path(d, path)
    else:
        writer.startWriter("COMMAND", LEVEL_INFO, u"Runtime Command 검사",
                           u"이 앱은 critical한 함수 'Runtime.getRuntime().exec(\"...\")'를 사용하고 있지 않습니다.", ["Command"])
...
다음 메서드의 코드를 확인하십시오.:"""
        ...
        writer.startWriter("SSL_CN2", LEVEL_INFO, u"SSL 구현 검사(필드에서 호스트 이름 확인)",
                           u"심각한 취약성인 \"ALLOW_ALL_HOSTNAME_VERIFFER\" 필드 설정 또는 \"AllowAllHostnameVerifier\" 클래스 인스턴스를 찾을 수 없습니다.",
                           ["SSL_Security"])
...
안전하지 않은 코드를 제거하십시오.:"""
        ...
        writer.startWriter("SSL_CN3", LEVEL_INFO, u"SSL 구현 검사(필드에서 호스트 이름 확인)",
    ...
    if list_HttpHost_scheme_http:
        writer.startWriter("SSL_DEFAULT_SCHEME_NAME", LEVEL_CRITICAL, u"SSL 구현 검사(HttpHost)",
                           u"이 앱은 \"HttpHost\"를 사용하지만 기본 체계는 \"Http\" 또는 \"HttpHost\"입니다.DEFAULT_SCHEME_NAME(http)\"입니다. \"https\"로 변경하십시오.:",
                           ["SSL_Security"])

        for i in list_HttpHost_scheme_http:
            writer.show_Path(d, i)
    else:
        writer.startWriter("SSL_DEFAULT_SCHEME_NAME", LEVEL_INFO, u"SSL 구현 검사(HttpHost)",
                           u"HttpHost위한 DEFAULT_SCHEME_NAME 검사: OK", ["SSL_Security"])
    ...
    if list_webviewClient:
        ...
        writer.startWriter("SSL_WEBVIEW", LEVEL_INFO, u"SSL 구현 검사(WebView위한 WebViewClient)",
                           u"\"WebViewClient\"(MITM 취약성)의 중요한 사용을 탐지하지 못했습니다.", ["SSL_Security"])
    ...
    if list_setJavaScriptEnabled_XSS:
        writer.startWriter("WEBVIEW_JS_ENABLED", LEVEL_WARNING, u"WebView XSS 공격 검사",
                           u"WebView에서 \"set JavaScriptEnabled(true)\"를 발견했으며 잠재적인 XSS 공격에 노출될 수 있습니다. 웹 페이지 코드를 주의 깊게 확인하고 출력을 삭제하십시오.",
                           ["WebView"])
        for i in list_setJavaScriptEnabled_XSS:
            writer.show_Path(d, i)
    else:
        writer.startWriter("WEBVIEW_JS_ENABLED", LEVEL_INFO, u"WebView XSS 공격 검사",
                           u"WebView에서 \"set JavaScriptEnabled(true)\"를 검색하지 못했습니다.", ["WebView"])
 ...
 (2)http://android-developers.blogspot.tw/2011/09/androids-http-clients.html"""
    ...
    if (int_min_sdk is not None) and (int_min_sdk < 11):
        ...
        if path_SQLiteDatabase_beginTransactionNonExclusive:
            ...
            output_string.append(u"\"SQLiteDatabase\"에서 \"beginTransactionNonExclusive\"를 사용 중이지만 minSdk는 " + str(
                int_min_sdk) + u"아래를 지원합니다.")
            output_string.append(
                u"\"beginTransactionNonExclusive\"는 API < 11에서 지원되지 않습니다. 이전 버전의 Android에서 \"beginTransaction\"을 사용해야 합니다.")
            output_string.append(
                u"참조: http://developer.android.com/reference/android/database/sqlite/SQLiteDatabase.html#beginTransactionNonExclusive()")
            writer.startWriter("DB_DEPRECATED_USE1", LEVEL_CRITICAL, u"SQLiteDatabase Transaction 반대 검사",
                               output_string.get(), ["Database"])

            writer.show_Paths(d, path_SQLiteDatabase_beginTransactionNonExclusive)
        else:
            writer.startWriter("DB_DEPRECATED_USE1", LEVEL_INFO, u"SQLiteDatabase Transaction 반대 검사",
                               u"\"SQLiteDatabase:beginTransactionNonExclusive\"을 사용하고 있지 않습니다.", ["Database"])
    else:
        writer.startWriter("DB_DEPRECATED_USE1", LEVEL_INFO, u"SQLiteDatabase Transaction 반대 검사",
                           u"설정된 minSdk > = 11이므로 \"SQLiteDatabase:beginTransactionNonExclusive\" 검사를 무시합니다.",
                           ["Database"])
    ...
    if list_NDK_library_classname_to_ndkso_mapping:
        writer.startWriter("NATIVE_LIBS_LOADING", LEVEL_NOTICE, u"기본 라이브러리 로드 검사",
                           u"네이티브 라이브러리 로드 코드(System.loadLibrary(...))가 발견되었습니다.:")
        ...
        if args.extra == 2:  # The output may be too verbose, so make it an option
            ...
            for class_name, method_names in dic_native_methods_sorted.items():
                if class_name in dic_NDK_library_classname_to_ndkso_mapping:
                    writer.write(u"Class: %s (로드된 NDK 파일: %s)" % (
    ...
    if is_using_Framework_MonoDroid:
        writer.startWriter("FRAMEWORK_MONODROID", LEVEL_NOTICE, u"암호화 프레임워크 - MonoDroid",
    ...
    if paths_ExternalStorageAccess:
        writer.startWriter("EXTERNAL_STORAGE", LEVEL_WARNING, u"외부 스토리지 액세스",
                           u"외부 스토리지 액세스 발견(외부 스토리지에 중요한 파일을 쓰지 마십시오):")
...
(6)https://cureblog.de/2013/11/cve-2013-6271-remove-device-locks-from-android-phone/ """
        ...
        writer.startWriter("FRAGMENT_INJECTION", LEVEL_CRITICAL, u"Fragment 취약성 검사", output_string, None,
        ...
        if list_Fragment_vulnerability_NonMethod_classes:
            if int_target_sdk >= 19:
                ...
                writer.write(
                    u"Android 4.4에서 예외 발생을 방지하려면 모든 \"PreferenceActivity\" 클래스에서 'isValidFragment' 메서드를 재정의해야 합니다.")
                for i in list_Fragment_vulnerability_NonMethod_classes:  # Notice: Each element in the list is NOT method, but String
                    writer.write("    " + i)
            else:
                # You must override. Otherwise, it always throws Exception
                writer.write(
                    u"\"PreferenceActivity\" 클래스는 'isValidFragment' 메서드를 재정의하지 않으므로 취약할 수 있습니다(기본 설정 활동에 조각을 로드하지 않은 경우에도 'isValidFragment' 메서드를 재정의하고 이후 변경 시 앱을 보호하려면 \"false\"만 반환하십시오).")
                for i in list_Fragment_vulnerability_NonMethod_classes:  # Notice: Each element in the list is NOT method, but String
                    writer.write("    " + i)
        ...
        if list_Fragment_vulnerability_Method_OnlyReturnTrue_methods:
            ...
            writer.write(
                u"(예시 코드: http://stackoverflow.com/questions/19973034/isvalidfragment-android-api-19/20139823#20139823)")
        ...
        writer.startWriter("FRAGMENT_INJECTION", LEVEL_INFO, u"Fragment 취약성 검사",
                           u"\"Fragment\"가 \"Preference Activity\" 또는 \"Sharlock Preference Activity\"에 동적으로 로드되는 취약성을 감지하지 못했습니다.",
    ...
    if list_implicit_service_components:
        writer.startWriter("PERMISSION_IMPLICIT_SERVICE", LEVEL_CRITICAL, u"암묵적 서비스 검사",
                           u"""앱의 보안을 위해 서비스를 시작할 때 항상 명시적 의도를 사용하고 서비스에 대한 의도 필터를 선언하지 마십시오.\n암묵적 의도를 사용하여 서비스를 시작하면 어떤 서비스가 의도에 응답할지 확신할 수 없고 사용자는 어떤 서비스가 시작되는지 볼 수 없기 때문에 보안 위험이 있습니다.\nReference: http://developer.android.com/guide/components/intents-filters.html#Types""",
                           ["Implicit_Intent"])
    ...
    if has_SSE_databases:
        writer.startWriter("DB_SEE", LEVEL_NOTICE, u"Android SQLite 데이터베이스 암호화 (SQLite Encryption Extension (SEE))",
                           u"이 앱은 Android(http://www.sqlite.org/android)에서 SQLite Encryption Extension(SEE)을 사용하여 데이터베이스를 암호화하거나 암호 해독합니다.",
                           ["Database"])

    else:
        writer.startWriter("DB_SEE", LEVEL_INFO, u"Android SQLite 데이터베이스 암호화 (SQLite Encryption Extension (SEE))",
                           u"이 앱은 Android(http://www.sqlite.org/android)에서 데이터베이스를 암호화하거나 암호 해독하기 위해 SQLite Encryption Extension(SEE)을 사용하지 않습니다.",
                           ["Database"])
    ...
    if result_sqlite_encryption:
        ...
        writer.startWriter("HACKER_DB_KEY", LEVEL_INFO, u"Android SQLite 데이터베이스 암호화용 키",
                           u"SQLite 데이터베이스를 암호화하기 위해 대칭 키(PRAGMA 키)를 사용하는 것을 찾지 못했습니다(사용할 수도 있지만 찾지 못했습니다).",
                           ["Database", "Hacker"])
    ...
    if path_Device_id:
        ...
        writer.startWriter("SENSITIVE_DEVICE_ID", LEVEL_WARNING, u"IMEI 및 Device ID 가져오기",
                           u"""이 앱에는 "device ID(IMEI)"를 가져오는 코드가 있고 "TelephonyManager.getDeviceId()" 접근 방식에는 문제가 있습니다.\n1. 비 전화: 전화 통신 하드웨어가 없는 Wi-Fi 전용 장치 또는 음악 플레이어에는 이러한 종류의 고유 식별자가 없습니다.\n2. 지속성: 이 기능이 있는 기기에서는 기기 데이터 삭제 및 공장 초기화 후에도 지속됩니다.\n이 상황에서 앱이 이를 동일한 장치로 간주해야 하는지 여부는 전혀 명확하지 않습니다.\n3. 권한: READ_PHONE_STATE 권한이 필요합니다.\n4. 버그: 구현에 버그가 있고 가비지(예: 0 또는 별표)를 반환하는 프로덕션 전화기의 몇 가지 인스턴스를 보았습니다.\n장치의 고유 ID를 얻으려면 다음 문서에서 "설치" 프레임워크를 사용하는 것이 좋습니다.\n참조: http://android-developers.blogspot.tw/2011/03/identifying-app-installations.html""", ["Sensitive_Information"])
        ...
        writer.startWriter("SENSITIVE_DEVICE_ID", LEVEL_INFO, u"IMEI 및 Device ID 가져오기",
                           u"이 앱이 \"TelephonyManager.getDeviceId()\" 접근 방식으로 \"Device ID(IMEI)\"를 가져오는 것을 감지하지 못했습니다.",
    ...
    if list_android_id:
        ...
        writer.startWriter("SENSITIVE_SECURE_ANDROID_ID", LEVEL_INFO, u"ANDROID_ID 가져오기",
                           u"이 앱이 64비트의 번호 \"Settings.Secure.ANDROID_ID\"를 가져오는 것을 감지하지 못했습니다.",
                           ["Sensitive_Information"])
    ...
    if sharedUserId_in_system:
        writer.startWriter("SHARED_USER_ID", LEVEL_NOTICE, u"AndroidManifest sharedUserId 검사",
                           u"이 앱은 \"system(uid=1000)\" 권한이 필요한 \"android.uid.system\" sharedUserId를 사용합니다. 사용자의 기기에 성공적으로 설치하려면 개발업체의 키 저장소 또는 Google의 키 저장소로 서명해야 합니다.",
                           ["System"])
    else:
    ...
    if path_getInstallerPackageName:
        writer.startWriter("HACKER_INSTALL_SOURCE_CHECK", LEVEL_NOTICE, u"APK Installing Source 검사",
                           u"이 앱에는 코드 검사 APK 설치 프로그램 소스(예: Google Play, Amazon 등)가 있습니다. 앱이 공격자에 의해 해킹되었는지 확인하는 데 사용할 수 있습니다.",
                           ["Hacker"])
